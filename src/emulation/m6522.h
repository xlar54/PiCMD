...
*/
	...
	// Shift register outputs (for fast serial on the CMD HD's U10).
	// In the shift out modes CB1 carries the shift clock and CB2 the data.
	inline bool GetShiftClockOut() const { return cb1OutputShiftClock != 0; }
	inline bool GetShiftDataOut() const { return cb2Shift != 0; }
	inline unsigned char GetShiftMode() const { return (auxiliaryControlRegister & ACR_SHIFTREG_CTRL) >> 2; }
private:
