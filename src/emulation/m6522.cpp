...
void m6522::Reset()
{
	...
	bitsShiftedSoFar = 0;
	cb1OutputShiftClock = 1;	// shift clock idles high
...
void m6522::Execute()
{
	...
	switch (shiftMode)
	{
		...
		case 6:		// 110 = shift out by phi2
			if (!(bitsShiftedSoFar & 8))
			{
				// The shift clock on CB1 runs at phi2/2; data on CB2 changes
				// on the falling edge of the shift clock. (Used by the CMD HD
				// for fast serial output.)
				cb1OutputShiftClock = !cb1OutputShiftClock;
